...
function s_addPatternImgOtherPallete( imageType, imgDirPath, addImgEx, pathOut, pallete,  countAdd, between, typeArrange, maxAlphaIn, bRandAlphaIn, randXOffset, randYOffset )
	...
	local arrColor = s_getpallete( pallete )		
	...
	if typeArrange == 0 then 
		...
		while xCount < countAdd do	
			...
			while yCount < countAdd do				
				...
				local picAdd = s_interpToColor( fileFullPath,nil, cRRand,cGRand,cBRand, 1.0, false )
	...
	util.msg("***s_addPatternImgOtherPallete finished")
...
s_addPatternImgOtherPallete( "seamless", "c:/foto/pattern/2", "png", PathOut, pallete, count, between, patterType, 1.0, false, xRand, yRand )