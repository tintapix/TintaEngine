...
-- smooth
local kernelName = "gpu/clear_back.cl"
-- monochrome sharp
--local kernelName = "gpu/clear_back_r.cl"
...
if smooth == true then
	...
	ocl.create( kernelName, "rough" )
end


local PathIn = "c:/test/test5.png"
...
--  if( distance(v1,v2) < factor ){
ocl.setvalue(kernelName, 200.0) -- max 441.6729 (black - white)

ocl.setvalue(kernelName, image.channels( image.selected()))

if  smooth == true then
	ocl.setvalue( kernelName, 4 ) -- default 0 // v(0...255) < threshold ? 0 : v;
end
