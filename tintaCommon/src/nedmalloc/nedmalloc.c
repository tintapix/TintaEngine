...
void * nedmemalign(size_t alignment, size_t mbytes) THROWSPEC { return nedpmemalign(0, alignment, mbytes); }
#if !NO_MALLINFO
...
struct threadcacheblk_t
{	/* Keep less than 16 mbytes on 32 bit systems and 32 mbytes on 64 bit systems */
...
void * nedpmemalign(nedpool *p, size_t alignment, size_t mbytes) THROWSPEC
{
	void *ret;
	threadcache *tc;
	int mymspace;
	GetThreadCache(&p, &tc, &mymspace, &mbytes);
	{	/* Use this thread's mspace */
        GETMSPACE(m, p, tc, mymspace, mbytes,
                  ret=mspace_memalign(m, alignment, mbytes));
	}
	return ret;
}
#if !NO_MALLINFO
