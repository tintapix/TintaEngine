...
namespace Tinta {
 ...
 typedef tintaPixel32b color_type;   
...
virtual		void   mixImage(const tintaUInt8Image *imgSecond, int x_offset, int y_offset, m_float32 factor = 1.f);

//injects other image - alpha interpolating. color assigning
virtual		void   mixChannel(const tintaUInt8Image *imgSecond, int x_offset, int y_offset, m_uint8 channel, m_float32 alphaFactor = 1.f);
...
virtual void		setPixel(const color_type &new_value);
...
virtual void		setChannel( int ch, m_uint8 value );
...
virtual const m_uint8 *getData();

virtual m_uint8       *getDataEx();
...
virtual     ImgChannels channels() const;
